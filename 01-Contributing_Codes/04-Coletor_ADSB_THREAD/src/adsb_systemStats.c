...
void* saveSystemStats(){
    ...
    while(1){
        ...
        fprintf(fcpu,"%d,%d,%lf,%ld\n",COLLECTOR_C,HARD_PC,CPU_usage(),now);
        fprintf(fmem,"%d,%d,%lf,%ld\n",COLLECTOR_C,HARD_PC,MEM_usage(),now);
...
void saveDecodingTime(double decTime){
    ...
    fprintf(fdec,"%d,%d,%lf,%ld\n", COLLECTOR_C, HARD_PC, decTime,getCurrentTimeMilli());
...
void saveReceivedMessage(char *msg, char *path){
    ...
    fprintf(fmsg,"%d,%d,%s,%ld\n", COLLECTOR_C, HARD_PC, msg,getCurrentTimeMilli());
    fclose(fmsg);
}